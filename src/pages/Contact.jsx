...
const Contact = () => {
  ...
  const { officialContacts, organizationLeaders } = useMemo(() => {
    return {
      officialContacts: allContacts.filter((contact) => contact.type === 'official'),
      organizationLeaders: allContacts.filter((contact) => contact.type === 'leader'),
    };
  }, [allContacts]);
        ...
        {loading ? (
          ...
          <Card>
            <Card.Body className="text-center py-16">
              <SearchIcon className="w-20 h-20 mx-auto text-gray-400 mb-6" />
              <p className="text-lg text-gray-600 dark:text-gray-400">
